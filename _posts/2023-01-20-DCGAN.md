...
$G$는 랜덤 노이즈를 입력으로 받아 가짜 이미지를 생성합니다. 여기서는 `ConvTranspose2d`, 즉 전치 합성곱(Transposed Convolution) 레이어를 사용하여 노이즈 벡터의 차원을 점차 늘려 이미지를 생성한다. BatchNorm2d는 각 레이어의 출력을 정규화하여 학습을 안정화하는 역할을 한다. ReLU 활성화 함수는 비선형인 특성을 추가하고, 마지막 레이어에서는 Tanh 함수를 사용하여 이미지 픽셀 값을 -1과 1 사이로 조정한다.
