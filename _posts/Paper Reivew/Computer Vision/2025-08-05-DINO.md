...
# **[논문 리view] Emerging Properties in Self-Supervised Vision Transformers** (DINO)
...
$$
\min_{\theta_s} \sum_{x \in \{x^{g}_1, x^{g}2\}} \sum_{\substack{x’ \in V \\ x’ \neq x}} H(P_t(x), P_s(x’))
$$
...
$$
\bar{\theta}t = \frac{1}{t} \sum{i=1}^t \theta_i
$$
